...
class SDDS:
     ...
     def load(self, input):
          ...
          try:
               ...
               self.parameterData = range(numberOfParameters)
               self.columnData = range(numberOfColumns)
               for i in range(numberOfParameters):
...
#          try:
          if False:
               ...
               if numberOfParameters != len(self.parameterData):
                    raise StandardError, "unmatched parameterName and parameterData"
               if numberOfColumns != len(self.columnData):
                    raise StandardError, "unmatched columnName and columnData"
               if numberOfParameters != len(self.parameterDefinition):
                    raise StandardError, "unmatched parameterName and parameterDefinition"
               if numberOfColumns != len(self.columnDefinition):
                    raise StandardError, "unmatched columnName and columnDefinition"
               ...
               for i in range(numberOfParameters):
                    if pages != len(self.parameterData[i]):
                         raise StandardError, "unequal number of pages in parameter data"
               for i in range(numberOfColumns):
                    if pages != len(self.columnData[i]):
                         raise StandardError, "unequal number of pages in column data"
               for page in range(pages):               
                    ...
                    for i in range(numberOfColumns):
                         if rows != len(self.columnData[i][page]):
                              raise StandardError, "unequal number of rows in column data"
          ...
          try:
               ...
               if sddsdata.WriteLayout(self.index) != 1:
                    sddsdata.PrintErrors(self.SDDS_EXIT_PrintErrors)
#               print sddsdata.parameterData
               #write SDDS data
               for page in range(pages):               
                    rows = 0
                    if numberOfColumns > 0:
#                         rows = len(self.columnData[0][page])
                         rows = len(self.columnData[0])
...
#                         if sddsdata.SetColumn(self.index, i, self.columnData[i][page]) != 1:
                         if sddsdata.SetColumn(self.index, i, self.columnData[i]) != 1:
                              sddsdata.PrintErrors(self.SDDS_EXIT_PrintErrors)
