...
class RbStatistics6D:
    def __init__(self):
        return

    def calcAverages6D(self, array6D):
        return sp.average(array6D, axis=1)

    def subtractAverages6D(self,array6D):
        averages6D = self.calcAverages6D(array6D)
        for nLoop in range(6):
            array6D[nLoop,:] -= averages6D[nLoop]
        return

    def calcVariance6D(self,array6D):
    ...
    def calcRmsValues6D(self,array6D):
        return sp.sqrt(self.calcVariance6D(array6D))

    def normalizeRmsValues6D(self,array6D):
        invRmsValues6D = 1. / self.calcRmsValues6D(array6D)
        for nLoop in range(6):
            array6D[nLoop,:] *= invRmsValues6D[nLoop]
        return

    def calcMinValues6D(self, array6D):
        return np.min(array6D, axis=1)

    def calcMaxValues6D(self, array6D):
        return np.max(array6D, axis=1)

    def calcCorrelations6D(self,array6D):
        ...
        npoints = array6D.shape[1]
        averages6D = self.calcAverages6D(array6D)
        variance6D = self.calcVariance6D(array6D)
    ...
    def eraseCorrelations6D(self,array6D):
        npoints = array6D.shape[1]
        sigmaM = self.calcCorrelations6D(array6D)        
        eigVals, eigVecs = self.jacobiEigenSolver6D(sigmaM)
    ...
    def jacobiEigenSolver6D(self, sigma6D):
        # Setup
        ...
        message = "Too many iterations in routine jacobiEigenSolver6D"
