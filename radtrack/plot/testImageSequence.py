...
class AxesSequence(object):
    ...
    def on_keypress(self, event):
        print 'on_keypress called; event = ', event
        if event.key == 'right':
            self.next_plot()
        elif event.key == 'left':
            self.prev_plot()
    ...
    def next_plot(self):
        print 'next_plot called; _i = ', self._i
    ...
    def prev_plot(self):
        print 'prev_plot called; _i = ', self._i
        if self._i > 0:
            self.axes[self._i].set_visible(False)
            self.axes[self._i-1].set_visible(True)
            self._i -= 1
    ...
    def show(self):
        print 'show called; axes[0] = ', self.axes[0]
