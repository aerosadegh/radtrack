...
#        Ny = int(self.ui.tableWidget.item(2,0).text())
    ...
    def GetPrecision(self,dialog):
        ...
        self.fieldInterpMeth = 4 #Magnetic Field Interpolation Method, to be entered into 3D field structures below (to be used e.g. for trajectory calculation):
    ...
    def srwbuttonThick(self):     
        ...
        (Xtrajectory,Ytrajectory,ctMesh)=self.CalcTraj(und,magFldCnt, self.arPrecPar, self.fieldInterpMeth)
        f = open('Trajectory.txt', 'w')
        f.write(str(Xtrajectory))
        f.write(str(Ytrajectory))
        f.write(str(ctMesh))
