...
#Defining parameters for analytic calculation
...
wfr.mesh.zStart = 0.5 #Longitudinal Position [m] at which Electric Field has to be calculated, i.e. the position of the first optical element
...
#OpElement.append(SRWLOptA("r", "a", 0.01, (SlitSeparation+2*Aperture)/2.0, 0.0, -(SlitSeparation+2*Aperture)/4.0))
OpElement.append(SRWLOptA("r", "a", 0.01, SlitSeparation+2*Aperture, 0.0, 0.0))
ppOpElement.append([0, 0, 1.0, 0, 0, 1.0, 1.0, 1.0, 1.0])
...
#    thx=2.0*(i-NptsOut/2.0+0.5)*Siz/NptsOut
#    thx=1.0*(i-size(arI1y)/2.0+0.0)*(wfr.mesh.yFin-wfr.mesh.yStart)/size(arI1y)
    thx=2.0*(i-wfr.mesh.ny/2.0)*wfr.mesh.yFin/wfr.mesh.ny
    yC.append(thx)
    xA=3.1415*Aperture*sin(thx)/lam
    xS=3.1415*SlitSeparation*sin(thx)/lam
    Inte.append((sin(xA)/(xA+1e-12)*cos(xS))**2)
#    Inte.append(((2*jv(1, xA)/(xA+1E-8))**2)*(cos(xS))**2)
#    yC.append(thx)
#    yC.append((wfr.mesh.yFin-wfr.mesh.yStart)/size(arI1y)*i)
