...
class fodocell:
    ...
    def get_twiss_parameters(self):
        if self.f and self.Lu:
            phi = np.arccos(self.transfer_matrix[0,0])
            sinphi = np.sin(phi)
            betax  = self.transfer_matrix[1,0]/sinphi
            gammax = -self.transfer_matrix[0,1]/sinphi
            alphax = (self.transfer_matrix[0,0]-
                      self.transfer_matrix[1,1])/(2.*sinphi)

            betay  = self.transfer_matrix[3,2]/sinphi
            gammay = -self.transfer_matrix[2,3]/sinphi
    ...
    def plot_beta_function(self):
        ...
        sArray = np.arange(0, self.Lu, 0.1*avgbeta)

        betaXArray   = 1./gammax + gammax*(sArray - alphax/gammax)**2
        betaYArray   = 1./gammay + gammay*(sArray - alphay/gammay)**2
        betaXArray   = np.append(betaXArray, betaXArray[::-1])
        betaYArray   = np.append(betaYArray, betaYArray[::-1])

        sArray = np.append(sArray, sArray+self.Lu)
